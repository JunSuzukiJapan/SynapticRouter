...
| 🧠 5 | [シナプス破壊実験](./notebooks/05_lesion_experiment_demo.ipynb) ✨ | 特定シナプス破壊時のタスク間影響を観察 | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/05_lesion_experiment_demo.ipynb) |
...
| 🧠 12 | [仮想ニューロンの創発](./notebooks/12_virtual_neuron_experiment.ipynb) | ドメインごとの仮想ニューロンの自律的な創発を観察 | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/12_virtual_neuron_experiment.ipynb) |
| 🧬 13 | [仮想ニューロンのホットスワップ](./notebooks/13_virtual_neuron_hotswap.ipynb) | 仮想ニューロン単位での安全な完全アンラーニング | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/13_virtual_neuron_hotswap.ipynb) |
| 🔬 14 | [削除単位の比較実験](./notebooks/14_compare_deletion_units.ipynb) | シナプス単位 vs ニューロン単位の削除影響比較 | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/14_compare_deletion_units.ipynb) |
| 📊 15 | [シナプス数と忘却閾値](./notebooks/15_capacity_hypothesis_experiment.ipynb) | シナプス容量とアンラーニングの安全性の関係を検証 | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/15_capacity_hypothesis_experiment.ipynb) |
| 🚀 16 | [Lazy Routingの改善](./notebooks/16_lazy_routing_prevention_experiment.ipynb) | Capacity LimitとEntropy制御による死んだシナプスの削減 | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/16_lazy_routing_prevention_experiment.ipynb) |
| 🔄 17 | [Routing Fallback](./notebooks/17_routing_fallback_experiment.ipynb) | 情報欠落を防ぐシナプス再割り当てと精度への影響を検証 | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/17_routing_fallback_experiment.ipynb) |
...
> 初期の検証（少ステップでのコピー・タスク）では、SRAがTransformerよりも早くValidation Lossを下げるケースが観察されていますが、現時点では小規模タスク上の予備結果です。
>
> 今後、ステップごとの速度（スループット）やVRAM効率の改善を含む、詳細なベンチマーク結果をここに追記する予定です。

## 🔬 現在の研究スコープ

このリポジトリは、ChatGPT級の汎用LLMを直接構築することではなく、**動的ルーティング、モジュール専門化、Hot-Swap、タスク間干渉の抑制**を小〜中規模実験で検証する研究プロトタイプです。したがって、各レポートの結果は現時点では「観測された現象」として扱い、今後はベースライン比較、複数seed、アブレーション、再現可能な評価スクリプトを優先して整備します。

開発方針は [`docs/dev/SRA_development_roadmap.md`](./docs/dev/SRA_development_roadmap.md) にまとめています。

## 🧪 実験・分析レポート (Experiments & Analysis)

- [アルゴリズム的推論におけるマルチタスク学習とルーティング分析](./docs/routing_analysis_algorithmic.md)
  - 小規模なアルゴリズムタスクで、タスク種別に応じたルーティング分化が見られるかを分析した予備レポートです。
- [異種ドメイン言語モデリングにおけるルーティング分析 (Code / Math / Text)](./docs/routing_analysis_language.md)
  - コード、数式、自然言語の文字レベルモデリングで、ドメインごとのシナプス使用分布を分析した予備レポートです。
- [多言語機械翻訳におけるルーティング分析（英・仏・日）とゼロショット汎化](./docs/dev/multilingual_translation_routing_analysis.md)
  - 言語ペアや構文差に応じたルーティング分布の変化を分析し、未学習ペアで観察された出力傾向をまとめたレポートです。
- [Decision Transformer（強化学習）における知覚と方策のルーティング分析](./docs/dev/decision_transformer_routing_analysis.md)
  - グリッドワールド系タスクで、入力処理と行動選択に関わるルーティングがどのように分かれるかを分析したレポートです。
- [SRA Encoder-Decoder による多言語翻訳の予備検証](./docs/dev/sra_seq2seq_translation_analysis.md)
  - SRAを Encoder-Decoder 型に拡張し、実コーパス（opus100）を用いた 30,000 ステップの学習で定型表現の翻訳と BLEU の改善傾向を確認した予備レポートです。
