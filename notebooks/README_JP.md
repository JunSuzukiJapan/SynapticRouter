...
### 🎯 19. Zero-Shot Hard Routing (メタデータによる強制割り当て)
**ファイル:** [`19_zero_shot_hard_routing.ipynb`](./19_zero_shot_hard_routing.ipynb)

ルーターに学習させることなく、入力データのメタデータ（キーワードなど）に基づいて、特定のカスタムシナプス（電卓シナプスなど）へのトラフィックを100%に強制する仕組みを検証します。事前学習やファインチューニングなしに、外部のルールや分類器の判定結果をルーターに直接反映させる「Zero-Shot ルーティング」の挙動を確認できます。

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/19_zero_shot_hard_routing.ipynb)

---

### 🔄 20. Fine-tuning による自律的なルーティング学習
**ファイル:** [`20_routing_finetuning.ipynb`](./20_routing_finetuning.ipynb)

VectorDBなどのカスタムシナプスを追加した後、少量のデータセットを用いてモデルをファインチューニングする検証を行います。メタデータによる強制割り当てが難しい場合でも、損失関数にルーティング誘導を組み込むことで、ルーターのEmbeddingが更新され、モデルが自律的に適切なシナプスを選択するように適応していくプロセスを可視化します。
...
### 🧩 23. SRAとオープンソースLLMのネイティブ統合 (PoC)
**ファイル:** [`nb23_sra_llm_integration.ipynb`](./nb23_sra_llm_integration.ipynb)

SRAを「独立した言語モデル」としてだけでなく、「既存の強力なAIモデルや非学習ツールを安全に接続するためのマザーボード」として活用するパラダイムシフト（デュアルトラック戦略）の概念実証です。Llama 3アーキテクチャの軽量版（TinyLlama）を凍結されたシナプスとして用い、ネイティブな次元数でのシームレスなルーティング（ポン付け）が可能であることを確認します。

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/nb23_sra_llm_integration.ipynb)

---

### 🧪 24. ルーターアーキテクチャのベンチマーク
**ファイル:** [`24_router_architecture_benchmark.ipynb`](./24_router_architecture_benchmark.ipynb)

Notebook 23で見えた「Mean Poolingによる信号の希釈」という課題を解決するため、Baseline、Last-Token Router、MLP Router、Attention-based Pooling Routerを比較検証します。学習損失やルーティング確信度を可視化し、SRAに適したルーター構成を評価するベンチマークです。

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/24_router_architecture_benchmark.ipynb)

---

### 🧩 25. Last-Token Router と異種シナプスの統合検証
**ファイル:** [`25_integrated_heterogeneous_routing.ipynb`](./25_integrated_heterogeneous_routing.ipynb)

Notebook 24で有効性を確認したLast-Token Routerを、Neural（汎用LLM）、Rule-based（電卓）、Retrieval（VectorDB）という異種シナプスを束ねるマザーボードアーキテクチャに統合します。容量上限に達した際に安全に第2候補へ迂回するSemantic Fallbackも検証します。

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/25_integrated_heterogeneous_routing.ipynb)

---

### 💬 26. SRA チャットボット・デモ
**ファイル:** [`26_chatbot_demo.ipynb`](./26_chatbot_demo.ipynb)

Notebook 25のマザーボードアーキテクチャを利用し、ipywidgetsによるチャットUIを構築します。ユーザー入力に応じて、汎用LLM、電卓、VectorDBの各シナプスへ動的にルーティングし、SRAの応答生成を対話形式で体験できます。

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/26_chatbot_demo.ipynb)

---

### 🔀 27. 複数の公開モデルをSRAで束ねる — メリットは本当にあるか
**ファイル:** [`27_multimodel_open_llm_routing.ipynb`](./27_multimodel_open_llm_routing.ipynb)

Gemma・Llama・Qwenのような公開済みの小型LLMを、各モデルのtokenizer/chat templateに閉じた「テキスト入力→出力」のシナプスとして扱い、隠れ状態に触れずに束ねます。SentenceTransformerによるゼロショット意味ルーターで入力ごとに得意なモデルへ振り分け、混在ワークロード（コード/数学/QA）でルーティングが最良の単体モデルを上回るか（A）、多数決アンサンブルがコストに見合うか（B）を、HumanEval/GSM8K/MMLUのサブセットで検証します。

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JunSuzukiJapan/SynapticRouter/blob/main/notebooks/27_multimodel_open_llm_routing.ipynb)
