...
SRAモデルに対して「8つの異なるドメイン × 5種類の類似タスク = 全40タスク」を同時に学習させ、ルーターの重みを解析します。複数のタスクで共通して使われるシナプス（共通コアニューロン）や、特定のドメインでのみ使われるシナプス（特化ニューロン）といった「仮想ニューロン（Cell Assembly）」が自律的に形成される様子をヒートマップで視覚的に観察できます。
