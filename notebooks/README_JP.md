...
### 🧠 12. 仮想ニューロンの創発実験（Virtual Neuron Emergence）
**ファイル:** [`12_virtual_neuron_experiment.ipynb`](./12_virtual_neuron_experiment.ipynb)

SRAモデルに対して「8つの異なるドメイン × 5種類の類似タスク = 全40タスク」を同時に学習させ、ルーターの重みを解析します。ドメインごとに全く異なるシナプスの組み合わせが選択されることで、ネットワーク全体に「算術用」「暗号用」といった**独立した複数の仮想ニューロン（Cell Assembly）が同時に並立して創発する様子**をヒートマップで視覚的に観察できます。
